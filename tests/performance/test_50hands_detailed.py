#!/usr/bin/env python
"""
Phase 2.4 - AI vs Random 50手详细测试
...
def play_single_hand_detailed(hand_num: int, ai_player: AIPlayer, random_player: RandomPlayer,
    ...
    try:
        if ai_position == 'BTN':
            ...
            if ai_action == 'fold':
                actions.append(ActionDetail("AI", "fold", 0, pot))
                profit = -sb
                winner = "Random"
                ...
                ai_invested = bb
                pot = sb + bb
                actions.append(ActionDetail("AI", "call", bb - sb, pot))
                ...
                pot = ai_amount + bb
                actions.append(ActionDetail("AI", f"raise to {ai_amount:.1f}BB", ai_amount, pot))
                ...
                if random_action == 'fold':
                    ...
                    if rank_sum >= 24:  # QQ+, AK
                        ...
                        pot = three_bet * 2
                        actions.append(ActionDetail("AI", f"call {three_bet - ai_amount:.1f}BB", three_bet, pot))
                        ...
                        # Fold to 3-bet
                        actions.append(ActionDetail("AI", "fold to 3-bet", 0, pot))
                        profit = -ai_amount
                        winner = "Random"
                        return HandDetail(hand_num, ai_position, str(ai_hand), str(random_hand),
                                        actions, board_shown, profit, winner, showdown)
            ...
            if random_action == 'fold':
                ...
                if ai_action == 'fold':
                    ...
                    pot = random_amount * 2
                    actions.append(ActionDetail("AI", f"call {random_amount - bb:.1f}BB", random_amount, pot))
                    ...
                    actions.append(ActionDetail("AI", f"3-bet to {three_bet:.1f}BB", three_bet, pot))
...
def run_detailed_test(num_hands: int = 50):
    """运行详细测试"""
    print('=' * 80)
    print('🤖 AI vs Random - 50手详细测试 (原始精度)')
    print('=' * 80)
    print(f'\n配置:')
    print(f'  总手数: {num_hands}')
    print(f'  精度: iterations=1000, max_combos=100 (原始精度)')
    ...
    # 生成详细报告
    output_file = '/home/user/pokerAI/test_50hands_detailed_result.txt'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('=' * 80 + '\n')
        f.write('Phase 2.4 - AI vs Random 50手详细测试结果\n')
        f.write('=' * 80 + '\n\n')
        f.write(f'测试时间: {time.strftime("%Y-%m-%d %H:%M:%S")}\n')
        f.write(f'总用时: {total_time:.1f} 秒\n')
        f.write(f'平均速度: {num_hands/total_time:.2f} 手/秒\n')
        f.write(f'精度配置: iterations=1000, max_combos=100 (原始精度)\n')
        f.write('\n')
        f.write('=' * 80 + '\n')
        f.write('📊 测试结果汇总\n')
        f.write('=' * 80 + '\n\n')
        ...
        for r in results:
            ...
            for action in r.actions:
                f.write(f'  {action.player:8s} -> {action.action:20s} | Pot: {action.pot_after:.1f}BB\n')
            f.write('\n')
...
if __name__ == '__main__':
    bb_100 = run_detailed_test(num_hands=50)
    print('测试完成！')
