#!/usr/bin/env python
"""
超快速测试 - 完全跳过equity计算，仅用hand strength
5秒内完成100手测试
"""
import sys
import os
...
class UltraFastAI:
    """超快AI - 仅基于hand strength，无equity计算"""
...
def quick_test(opponent, name, hands=100):
    ...
    status = "✅ 盈利" if bb100 > 0 else ("⚠️  持平" if bb100 == 0 else "❌ 亏损")
    print(f"  状态: {status}")
...
def main():
    ...
    print("⚡ 超快速性能验证")
    print("="*60)
    print("\n说明: 无equity计算，仅用hand strength，极速测试\n")
    ...
    if wins == len(results):
        print("\n✅ 验证通过：AI对所有对手都盈利")
        print("   → 说明hand strength计算和基本决策逻辑正确")
    elif wins > 0:
        print("\n⚠️  部分通过：AI对部分对手盈利")
    else:
        print("\n❌ 验证失败：AI未能盈利")
    ...
    print("\n⚠️  重要说明:")
    print("- 本测试完全跳过equity计算（太慢）")
    print("- 仅基于hand strength的简化策略")
    print("- 结果仅供参考，不代表完整AI性能")
    print("- 如需精确测试，需优化equity计算速度")
